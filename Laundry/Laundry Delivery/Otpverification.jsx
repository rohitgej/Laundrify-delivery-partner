...
const Otpverification = ({ route, navigation }) => {
  ...
  const handleVerify = async () => {
    ...
    if (otpCode.length === 4) {
      ...
      try {
        const response = await fetch(`${DOMAIN_NAME}api/runner-login`, requestOptions);
        const result = await response.json();

        if (response.ok) {
          const runner_id=result.runner_id;
          console.warn(runner_id);
          
          // Save login status
          await AsyncStorage.setItem('user_id', runner_id);
