  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        const pickupResponse = await fetch(`${DOMAIN_NAME}api/fetchPickupOrders`, requestOptions);
        const deliveryResponse = await fetch(`${DOMAIN_NAME}api/fetchDeliveryOrders`, requestOptions);
        ...
        {showPickup ? (
          <>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity onPress={() => navigation.navigate('OrderDetails', { orderId: item.order_id, orderType:'pick'})}>
          ...
          <>
            ...
            <FlatList
              ...
              keyExtractor={(item) => item._id}
              renderItem={({ item }) => (
                <TouchableOpacity onPress={() => navigation.navigate('OrderDetails', { orderId: item.order_id,orderType:'delivery'})}>
                  <OrderItem order={item} />
                </TouchableOpacity>
              )}
              contentContainerStyle={styles.listContainer}
