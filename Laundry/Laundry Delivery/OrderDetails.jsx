  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      ...
      try {
        ...
        console.log(result.order.delivery_status);
      } catch (error) {
        console.error(error);
      }
    };
    ...
    const fetchAddress = async () => {
      ...
      try {
        ...
        const addressWithCoordinates = result.user.addresses.find(
          addr => addr._id === orderDetails.address,
        );
  ...
  const handleSubmit = async () => {
    ...
    selectedImages.forEach((uri, index) => {
      formData.append('image', {
        ...
        name: `image${index}.jpg`, // File name
      });
    });
    formData.append('order_id', orderId);
    formData.append('field', orderType);
    ...
    try {
      const response = await fetch(
        `${DOMAIN_NAME}api/pickup-uploadImage`,
        requestOptions,
      );
      const result = await response.json();
      console.log(result);

      if (result.message === 'Image uploaded and converted to WebP format') {
        // Image uploaded successfully

        Alert.alert('Success', 'Images uploaded successfully');
        setIsUploading(false);
        setSelectedImages([]);
        // Navigate to Map or another screen
      } else {
  ...
  return (
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity style={styles.orderDetails} onPress={toggleExpand}>
          ...
          {isExpanded && (
            <>
              <View style={styles.expanded}>
                <View style={styles.subDetails}>
                  ...
                  <View style={styles.subDetailsDiv}>
                    ...
                    <Text style={styles.subDetailsText}>
                      {getAddressById(orderDetails.address)}
                    </Text>
                  </View>
                  <View style={styles.subDetailsDiv}>
              ...
              {deliveryStatus === 2 || deliveryStatus === 4 ? (
                <View style={styles.verifyButtonSec}>
                  <TouchableOpacity
                    style={styles.verifyButton}
                    onPress={() =>
                      
                      navigation.navigate('Map', {orderId, userCoordinates, orderType})
                    }>
                    <Text style={styles.verifyText}>Start your Ride</Text>
                  </TouchableOpacity>
                </View>
              ) : null}
            </>
        ...
        {deliveryStatus === 3 || deliveryStatus === 5 ? (
                 <FlatList
                 ...
                 ListFooterComponent={
                   ...
                   <View  style={styles.uploadButtonSec}>
